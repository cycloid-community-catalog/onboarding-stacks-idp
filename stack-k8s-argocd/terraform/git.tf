resource "github_repository" "manifests" {
  name        = "${var.cy_org}-${var.cy_env}-manifests"
  description = "ArgoCD Manifests repo managed by Cycloid for ${var.cy_env} environment in ${var.cy_org} org"
...
resource "github_repository_deploy_key" "manifests" {
  title      = "${var.cy_org}-${var.cy_env}-manifests"
...
resource "cycloid_credential" "ssh_key" {
  name                   = "${var.cy_org}-${var.cy_env}-manifests-git-ssh"
  description            = "SSH Key Pair used to access ArgoCD Manifests GitHub repo."
  path                   = "${var.cy_org}-${var.cy_env}-manifests-git-ssh"
  canonical              = "${var.cy_org}-${var.cy_env}-manifests-git-ssh"
