resource "github_repository" "manifests" {
  name        = "${var.cy_org}-manifests"
  description = "ArgoCD Manifests repo managed by Cycloid for ${var.cy_org} organization"
...
resource "cycloid_credential" "ssh_key" {
  name                   = "${var.cy_org}-manifests-git-ssh"
  description            = "SSH Key Pair used to access ArgoCD Manifests GitHub repo."
  path                   = "${var.cy_org}-manifests-git-ssh"
  canonical              = "${var.cy_org}-manifests-git-ssh"
