...
The plugin resolves the Cycloid component context (`org`, `env`, `component`) from the iframe request, logs into `https://argocd.<org>.demo.cycloid.io`, and embeds the Argo CD UI in a **nested iframe** under `/ui/*` (same architecture as plugin v1.9.x / v2.0.3).

The outer Cycloid iframe loads a thin shell at `/?org=…&component=…`. The inner iframe loads `${UI_MOUNT}${ARGOCD_ENTRY_PATH}` (default `/ui/applications/argocd/app-of-apps`). All Argo CD assets and API calls are proxied under `/ui/` so routing stays isolated from the Cycloid console URL.
...
| `ARGOCD_ENTRY_PATH` | `/applications/argocd/app-of-apps` | Path loaded in the inner `/ui` iframe |
...
Rebuild and upgrade to **2.1.0** (nested iframe architecture).
